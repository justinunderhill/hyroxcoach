...
`OPENAI_COACH_MODEL` selects the text model used for Phase 7 coach reviews (`api/services/ai_client.py`, `api/services/coach.py`). Also defaults to `gpt-4o-mini`; consider a stronger model here since coaching quality depends more on reasoning than the extraction path does.
